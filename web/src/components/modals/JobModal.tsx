...
import { Workflow__factory, NFT__factory, workflowFactoryAbi } from '../../../typechain';
...
const JobModal = ({ isOpen, onClose, data }: JobModalProps) => {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      const tx = await triggerMintFunc(data.workflowAddress.toLowerCase(), metadataCid);
                  ...
                  <div className='space-y-2'>
                    ...
                    {attestation ? (
                      <div className='px-3 py-2 bg-green-700 text-white rounded-md font-medium'>
                        <br />
                        TxHash:{' '}
                        <Link href={`https://pharosscan.xyz/tx/${attestation.txHash}`} target='_blank' rel='noopener noreferrer' className='underline'>
                          {attestation.txHash}
                        </Link>
                        <br />
                        Metadata:{' '}
